...
public class IcClientController implements ActionListener, TabClickListener {
	...
	private void clickOperate(BtnEnum btnEnum) {
		...
		// 保存
		case SAVE:{
			System.out.println("click doSave");
	...
	private void doSave(DefaultTableModel tableModel, JTable table) {
		...
		if (true) {
			...
			case SCHEDULING:
				...
				Map<String, List<Scheduling>> scMap = new HashMap<String, List<Scheduling>>();
				scMap.put("SAVE", new ArrayList<Scheduling>());
				scMap.put("DELETE", new ArrayList<Scheduling>());
				scMap.put("UPDATE", new ArrayList<Scheduling>());
				for (Object obj : dataList) {
					Scheduling tempscheling = (Scheduling) obj;
					if (tempscheling.getLineCode() == null || tempscheling.getLineCode().length() == 0) {
						scMap.get("SAVE").add(tempscheling);
					}
				}
				if (changeDataList.get("DELETE").size() > 0) {
					for (Object obj : changeDataList.get("DELETE")) {
						scMap.get("DELETE").add((Scheduling) obj);
					}
				}
				if (changeDataList.get("UPDATE").size() > 0) {
					for (Object obj : changeDataList.get("UPDATE")) {
						scMap.get("UPDATE").add((Scheduling) obj);
					}
				}
				message = schedulingService.save(scMap);
				break;
			default:
				break;
			}
	...
	private void doSearch() {
		...
		case SCHEDULING:
			...
			// 查询线路请求
			System.out.println("scheling view");
			SchedulingService schedulingService = ServiceFactory.getSchedulingService();
			List<Scheduling> schedulingResult = schedulingService.find(SchedulingCondition);
			mainView.changeTable(schedulingResult, currentModule);
			break;
		default:
			break;
		}
